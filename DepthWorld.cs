                ...
                for (int k = 0; k < Main.maxTilesX; k++) {
                    for (int i = 0; i < Main.maxTilesY; i++) {
                        if (Main.tile[k, i].type == mod.TileType("BarrenDirtTile")) {
                            // if tiles on the sides are empty
                            if (!Main.tile[k + 1, i].active() && !Main.tile[k, i - 1].active()) {
                                // if space above is clear
                                bool spaceAboveIsClear = true;
                                int blocksAbove = -1;
                                while (blocksAbove < 15 && spaceAboveIsClear) {
                                    int blocksBeside = -1;
                                    while (blocksBeside <= 1 && spaceAboveIsClear) {
                                        if (Main.tile[k + blocksBeside, i - blocksAbove].active()) {
                                            spaceAboveIsClear = false;
                                        }
                                        blocksBeside++;
                                    }
                                    blocksAbove++;
                                ...
                                if (spaceAboveIsClear) {
                                    if (WorldGen.genRand.Next(0, 16) == 2) {
                                        WorldGen.PlaceObject(k, i, mod.TileType("BarrenthornSapling"));
                                        WorldGen.GrowTree(k, i);
                                    }
                                }
                            }
                        }
